...
char *opcode_name[] = {"OP_NULL", "LOD", "LDX", "LDXB", "LDA", "LITI", "STO", "STOB", "STX", "STXB", "SUBI", "SUBF", "DIVI", "DIVF", "ADDI", "ADDF", "OFFSET", "MULI", "MULF", "MOD", "LSSI", "LSSF", "GTRI", "GTRF", "LEQI", "LEQF", "GEQI", "GEQF", "NEQI", "NEQF", "EQLI", "EQLF", "NOT", "OR", "AND", "CVTI", "CVTF", "JPC", "JPCR", "JMP", "JPT","JPTR", "INT", "INCI", "INCF", "DECI", "DECF", "SUP", "CAL", "ADDR", "RET", "MINUSI", "MINUSF", "CHK", "LDI", "LDIB", "POP", "POPB"};
...
void gen_expression (A_NODE *node){
	...
	case N_EXP_ASSIGN:
		...
		if (i == 1)
			...
			gen_code_i(STX, 0, 0);//i % 4 ? i / 4 + 1 : i / 4);
		break;
	default:
		gen_error(100, node->line);
		break;
	...
	switch (id->kind) {
		...
		case ID_FUNC:
			if (id->type->expr) {
				...
				gen_code_i(INT, 0, id->type->local_var_size); // local variable size
